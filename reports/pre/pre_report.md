...
작성자: 상혁 (2025440084) / 작성일: 2026-09-20 / 소스 커밋: `46b8079` / workspace: `LAB1.code-workspace` (템플릿 v2.0.1) / OS: `Windows 11 Home 10.0.26200` / Python: `Python 3.14.7` / 시뮬레이터: Icarus Verilog `Icarus Verilog version 12.0 (devel) (s20150603-1539-g2693dd32b)`
...
- 파형 캡처(VaporView): `evidence/pre/lab2_03_wave_zoom.png`(상승 에지 확대)
...
|---|---|---|---|---|
| 정상 코드 | `46b8079` | [normal.log](../../evidence/pre/lab2_03_normal.log) | 26 ns 기대 value=A, 실제 value=A. `LAB2_PASS register_pair checks=7` | 모든 검사 통과, 66 ns 종료. |
| 지정한 RTL 변경 | 미커밋 수정본(`46b8079` 기준, 로컬 실행) | [mod.log](../../evidence/pre/lab2_03_mod.log) | 26 ns 기대 value=A, 실제 value=3. `LAB2_FAIL transfer stored not live input time=26000`, `FATAL: sim/tb_register_pair.sv:12: check failed` | `transfer stored not live input` 검사가 변경을 발견했다(로그의 time은 ps 단위, 26000 ps = 26 ns). |
| 원래 코드로 복구 | `46b8079` | [recover.log](../../evidence/pre/lab2_03_recover.log) | 복구 후 전체 검사 재실행. `LAB2_PASS register_pair checks=7`, `$finish called at 66000 (1ps)` | PASS와 종료 시각이 정상 실행과 같고 새 VCD를 확인한다. |
